<?php

namespace MhsThemeExtension;

class MhsThemeExtension
{
    public function __construct() 
    {
        add_action( 'admin_menu', array( $this, 'addAdminMenu' ) );
        get_option('mhste_woo_product_info') == '0' ? false : add_action( $this->choosenWoocommerceHook(get_option('mhste_woo_product_info')), array($this, 'showSingleProductMessage') );
    }

    public function addAdminMenu() 
    {
        $mhsSettingsPage = add_menu_page( esc_html__('MHS Option', '_pluginname'), 'MHS Ext', 'manage_options', 'mhs-settings-page', array( $this, 'settingsPageHTML' ), 'data:image/svg+xml;base64,', 3 );
        add_submenu_page( 'mhs-settings-page', esc_html__('Settings', '_pluginname'), esc_html__('Settings', '_pluginname'), 'manage_options', 'mhs-settings-page', array( $this, 'settingsPageHTML' ) );
    }

    public function showSingleProductMessage() 
    { ?>
        <div class="mte-product-message">
        <p><?php echo esc_textarea( get_option( 'mhste_woo_product_message', '' ) ) ?></p>
        </div>
    <?php }

    public function saveSettingsInfo() 
    {
        if( wp_verify_nonce( $_POST['securityWooSettingsDataSave'], 'saveWooSettingsData' ) && current_user_can( 'manage_options' ) ) {
            update_option( 'mhste_woo_product_info', $this->sanitizeInput( $_POST['mhste_woo_product_info'] ) );
            update_option( 'mhste_woo_product_message', sanitize_text_field( $_POST['mhste_woo_product_message'] ) ); ?>

            <div class="updated"><p><?php esc_html_e( 'Your settings are succefuly saved.', '_pluginname' ) ?></p></div> 
        <?php } else { ?>
            <div class="error"><p><?php esc_html_e( 'Sorry, you do not have permission to perform that action.', '_pluginname' ) ?></p></div>
        <?php }
    }

    public function sanitizeInput($input) 
    {
        if($input != '0' && $input != '1' && $input != '2' && $input != '3' && $input != '4' && $input != '5') {
            return get_option('mhste_woo_product_info');
        }
        return $input;
    }

    public function choosenWoocommerceHook($option)
    {
        $wcHook = [
            false,
            'woocommerce_before_single_product',
            'woocommerce_single_product_summary',
            'woocommerce_before_add_to_cart_form',
            'woocommerce_after_add_to_cart_form',
            'woocommerce_after_single_product'
        ];
        return $wcHook[$option];
    }

    public function settingsPageHTML() 
    { ?>
        <div class="wrap">
            <h1><?php esc_html_e( 'Settings', '_pluginname' ) ?></h1>
            <?php if( isset($_POST['mhsep_woosettings_submitted']) && $_POST['mhsep_woosettings_submitted'] == 'true' ) $this->saveSettingsInfo(); ?>
            <form method="POST">
                <input type="hidden" name="mhsep_woosettings_submitted" value="true">
                <?php wp_nonce_field( 'saveWooSettingsData', 'securityWooSettingsDataSave' ) ?>
                <div class="mhsadmin-info mhsadmin-info-section">
                    <h2 class="mhsadmin-settings__title<?php if( !is_woocommerce_activated() ) echo ' mhsadmin-settings__disabled'; ?>"><?php esc_html_e( 'WooCommerce options', '_pluginname' ) ?></h2>
                    <?php if( !is_woocommerce_activated() ) echo '<p class="mhsadmin-settings__info">' . esc_html__( 'WooCommerce is not activated! Activate WooCommerce to enable this options.', '_pluginname' ) . '</p>'; ?>
                    <div class="mhsadmin-info__input-group">
                        <div class="mhsadmin-info__input-box" style="display:inline;margin-top:2rem;">
                            <label for="mhste_woo_product_info" <?php if( !is_woocommerce_activated() ) echo 'class="mhsadmin-settings__disabled"'; ?>><strong><?php esc_html_e( 'Message on product page:', '_pluginname' ) ?></strong></label> <select name="mhste_woo_product_info" <?php if( !is_woocommerce_activated() ) echo 'disabled'; ?>>
                                <option value="0" <?php selected( get_option('mhste_woo_product_info'), '0' ) ?>><?php esc_html_e( 'Disable', '_pluginname' ) ?></option>
                                <option value="1" <?php selected( get_option('mhste_woo_product_info'), '1' ) ?>><?php esc_html_e( 'Before single product', '_pluginname' ) ?></option>
                                <option value="2" <?php selected( get_option('mhste_woo_product_info'), '2' ) ?>><?php esc_html_e( 'In single product sumary box', '_pluginname' ) ?></option>
                                <option value="3" <?php selected( get_option('mhste_woo_product_info'), '3' ) ?>><?php esc_html_e( 'Before add to cart form', '_pluginname' ) ?></option>
                                <option value="4" <?php selected( get_option('mhste_woo_product_info'), '4' ) ?>><?php esc_html_e( 'After add to cart form', '_pluginname' ) ?></option>
                                <option value="5" <?php selected( get_option('mhste_woo_product_info'), '5' ) ?>><?php esc_html_e( 'After single product summary', '_pluginname' ) ?></option>
                            </select>
                        </div>
                    </div>
                    <div class="mhsadmin-info__input-group">
                        <div class="mhsadmin-info__input-box">
                            <label for="mhste_woo_product_message" <?php if( !is_woocommerce_activated() ) echo 'class="mhsadmin-settings__disabled"'; ?>><strong><?php esc_html_e( 'Message:', '_pluginname' ) ?></strong></label>
                            <textarea name="mhste_woo_product_message" id="mhste_woo_product_message" placeholder="<?php esc_html_e( 'Enter message here...', '_pluginname' ) ?>" <?php if( !is_woocommerce_activated() ) echo 'disabled'; ?>><?php echo esc_textarea( get_option( 'mhste_woo_product_message', '' ) ) ?></textarea>
                        </div>
                    </div>
                    <div class="mhsadmin-info__input-box mhsadmin-info__input-box--submit">
                        <input type="submit" name="submit" id="submit" class="button button-primary" value="<?php esc_html_e( 'Save changes', '_pluginname' ) ?>">
                    </div>
                </div>
            </form>
        </div>
    <?php }
}

$mhsThemeExtension = new MhsThemeExtension();
